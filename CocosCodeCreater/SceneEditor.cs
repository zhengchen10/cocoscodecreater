...
{
    public class SceneEditor : IEditer, IRefreshDocument
    {
        ...
        public SceneEditor(Scene obj,Global global)
        {
            this.global = global;
            scene = obj;// sceneName = obj.Name;
        ...
        private Node HitTest(Point location)
        {
            ...
            {
                Node node = scene.FindNodeByUUID(scene.Children[i]);
                if (node.Visible == false)
                    continue;
                RectangleF rect = node.getNodeDisplayRect();// new RectangleF(node.Location.X, node.Location.Y, node.Size.Width, node.Size.Height);
        ...
        public void OnPaint(Graphics g, Rectangle rect)
        {
                    ...
                    for(int j = 0; j < global.Project.Components.Count; j++)
                    {
                        if(global.Project.Components[j].UUID == component.RefUUID)
                        {
                            if (node.Visible)
                            {
                                ...
                                DrawTools.DrawContainer(global, global.Project.Components[j], g, new Point(r.X,r.Y), component.Size, seInfo);
                            }
                        }
                    }
                }
            ...
            if(scene.ScreenType == "横屏")
            {
                if(scene.Size.Width == 0|| scene.Size.Height == 0)
                    DrawTools.DrawRect(g, pen1, new Rectangle(0, 0, 480, 320), seInfo);
                else
                    DrawTools.DrawRect(g, pen1, new Rectangle(0, 0, scene.Size.Width, scene.Size.Height), seInfo);
            }
            else
            {
                if (scene.Size.Width == 0 || scene.Size.Height == 0)
                    DrawTools.DrawRect(g, pen1, new Rectangle(0, 0, 320, 480),seInfo);
                else
                    DrawTools.DrawRect(g, pen1, new Rectangle(0, 0, scene.Size.Width, scene.Size.Height), seInfo);
            }
            if (currentAction != null)
        ...
        public bool OnKeyDown(KeyEventArgs e)
        {
            ...
            if(e.KeyCode == Keys.Delete && selectNodes.Count >0)
            {
                String items = "";
                for(int i = 0; i < selectNodes.Count; i++)
                {
                    if (i > 0)
                        items = items + ",";
                    items = items + selectNodes[i].Name;
                }
                String message = "确认要删除 " + items + " 吗?";
                if (MessageBox.Show(message, "提示", MessageBoxButtons.OKCancel) == DialogResult.OK)
                {
                    for(int i = 0; i < selectNodes.Count; i++)
                    {
                        if (scene.RemoveNode(selectNodes[i]))
                        {
                            global.SceneExplorer.RemoveTreeNode(selectNodes[i].UUID);
                        }
                    }
                    selectNodes.Clear();
                    currentAction = null;
                    Refresh();
                    global.PropertyExplorer.PropertyGrid.SelectedObject = null;
