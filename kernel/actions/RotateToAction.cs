...
{
    public class RotateToAction : AbstractAction
    {
        public RotateToAction(ActionItem item)
        {
            Action = item;
        }
        public RotateToAction(ActionItem item, float duration, float rotateX, float rotateY, float rotateZ)
        {
            Action = item;
            Duration = duration;
            RotateX = rotateX;
        ...
        public override string CreateCode(HashSet<string> context, string paramName)
        {
            ...
            sb.Append("    CCActionInterval * ").Append(paramName).Append(" = CCRotateTo::create(").Append(Duration).Append(",Vec3(");
        ...
        {
            return "rotateTo(" + Duration + ",(" + RotateX + "," + RotateY + "," + RotateZ + "))";
        }
    }
}